# Spec: DID 配置和 `0x22` ReadDataByIdentifier
...
The system SHALL extend the simulator DID configuration model to describe readable fixed byte value DIDs.
...
- **AND** the model SHALL NOT require dynamic expression evaluation
...
#### Scenario: 拒绝无效固定字节值
- **GIVEN** a simulator configuration contains a DID entry with `valueEncoding` set to `hex`
...
#### Scenario: 不支持动态表达式 DID
- **GIVEN** task-013 is implemented
- **WHEN** DID configuration is inspected
- **THEN** the configuration SHALL NOT execute scripts
- **AND** it SHALL NOT evaluate dynamic expressions
- **AND** it SHALL NOT compute DID values from runtime formulas
...
The implementation SHALL remain limited to fixed byte DID configuration and UDS `0x22` ReadDataByIdentifier.
